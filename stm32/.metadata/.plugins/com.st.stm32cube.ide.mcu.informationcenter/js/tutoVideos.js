...
/**
 * Build the Eclipse intro runAction URL for a given YouTube URL.
 * The YouTube URL is URL-encoded so it is correctly passed as the "url" parameter.
 */
function buildIntroRunActionUrl(youtubeUrl) {
	const base =
		"http://org.eclipse.ui.intro/runAction" +
		"?pluginId=com.st.stm32cube.ide.mcu.informationcenter" +
		"&class=com.st.stm32cube.ide.mcu.informationcenter.DefaultApplicationOpenerAction" +
		"&url=";
	return base + encodeURIComponent(youtubeUrl);
}
...
function ProcessPlaylistJson(dataPlaylist, i, server){
		...
		if(server === "stmcu.com.cn"){
			...
			for(r = removeItems.length-1; r > -1 ; r--){
		...
		if(i == 0){
			...
			if(server === "stmcu.com.cn"){
				...
				const youtubeUrl =
				        ...
				        dataPlaylist[0].snippet.resourceId.videoId +
				        "&cc_load_policy=1&autoplay=1&list=" +
				        dataPlaylist[0].snippet.playlistId;

				    const href = buildIntroRunActionUrl(youtubeUrl);
				    areaarea.setAttribute('href', href);
			}
			// add on big pictures
...
function FillJSonPlaylist(id, i, server){
	...
	//baseurl = 'https://sw-center.st.com/stm32cubeide/videos/9795c7b13bce5cf3e3520c7c2792a26c15035832/';
	url = baseurl + id + '.json';
	$.ajax({
		url: url,
		dataType: 'json',
...
function loadPage(){
	...
	$.ajax({
		...
		let ytIde = buildIntroRunActionUrl("https://www.youtube.com/playlist?list=PL8n_tqwYdzVANEGjLs4C6ICkDNLhdsbmM");
		let ytMx = buildIntroRunActionUrl("https://www.youtube.com/playlist?list=PL8n_tqwYdzVDE2e7v8m0oTV6F9SYQFeqL");
		let ytDiscover = buildIntroRunActionUrl("https://www.youtube.com/playlist?list=PL8n_tqwYdzVAd_OxaKnfxBrvcGwR8kSJo");

		let cnIde = buildIntroRunActionUrl("https://www.stmcu.com.cn/video/sort/new/category/515");
		let cnMx = buildIntroRunActionUrl("https://www.stmcu.com.cn/video/sort/new/category/456?page=2");
		...
		parea.innerHTML =
			'<b>Network Error: </b>' +
			'Could not complete the operation due to an error.' +
			'<br><br>' +
			'You can still access the video series directly using the links below:' +
			'<br><br>' +

			'<b>YouTube</b>' +
			'<ul>' +
				'<li><a href="' + ytIde + '">STM32CubeIDE Tutorials</a></li>' +
				'<li><a href="' + ytMx + '">STM32CubeMX</a></li>' +
				'<li><a href="' + ytDiscover + '">Discover your STM32 with STM32CubeIDE</a></li>' +
			'</ul>' +

			'<b>stmcu.com.cn</b>' +
			'<ul>' +
				'<li><a href="' + cnIde + '">STM32CubeIDE Tutorials</a></li>' +
				'<li><a href="' + cnMx + '">STM32CubeMX</a></li>' +
			'</ul>';
